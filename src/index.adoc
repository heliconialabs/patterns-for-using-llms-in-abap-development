...
image::img/pattern-mcp-managed.svg[Managed MCP via ADT pattern]

This pattern keeps the MCP tool interface, but deploys the ADT MCP server as a centrally managed service instead of running it on each developer laptop. The server can be hosted on an internal platform, a dedicated server, or SAP BTP.

The main benefit is control. Authentication, logging, exposed tools, and write permissions are managed centrally, so organizations can offer read-only access by default and allow write access only for approved users, packages, or workflows.

This makes MCP more suitable for enterprise use, because the access model is no longer defined only by what an individual developer installs locally. However, the setup requires additional platform and operational work compared to a local MCP server.

Examples: ARC-1 deployed on own server or SAP BTP
...
=== Git / Off-Stack with read-only MCP

image::img/pattern-git-offstack-readonly-mcp.svg[Git / Off-Stack with read-only MCP pattern]

This pattern combines git-based off-stack development with read-only ADT MCP tools against the SAP system. The LLM edits only local files, and all writes still go through git, pull requests, review, CI/CD, and abapGit.

The MCP connection is used only for context: reading ABAP objects, inspecting dependencies, understanding system state, and doing impact analysis against the live SAP system. This gives the LLM better grounding without giving it direct write access to the system.

This is a practical option for teams that already want the safety of a git-based workflow, but still need the model to understand the actual SAP landscape instead of only the repository state.

Examples: Any editor, abapGit, ARC-1 in read-only mode
...
machine, so this pattern expects it is possible to quickly deploy a personal/team/project based ABAP system which lives for a few hours or few
...
short-lived branches merged into the main branch after CI and code review.
...
developers already automated their setup, write modularized code and use test doubles.
...
or in the worst case an invalid change reaches git. If something invalid reaches git, CI will most probably catch it. And even if CI does not
catch an invalid change, there is still a chance the human reviewer (or another LLM) will catch it while reviewing the change.
